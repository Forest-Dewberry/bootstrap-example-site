<template>
  <div>
    <v-row>
      <v-col>
        <v-label class="pl-2 pt-2 text-subtitle-2" @click="resetCurrent">Sprints
        ...
        <v-label class="pl-2 pt-2 text-subtitle-2" v-if="current[id]===undefined && form.show">New Sprint</v-label>
      </v-col>
    </v-row>
    <v-divider class="mt-2"></v-divider>
    <v-container v-if="form.show">
      <v-form v-model="form.valid" ref="$form">
        <v-text-field v-model="current.name" :rules="[required]" label="Name" type="text" density="compact"></v-text-field>
        <v-text-field v-model="current.description" label="Description" type="text" density="compact"></v-text-field>
        <v-text-field v-model="current.firstDay" :rules="[required]" label="First Day Of Sprint" type="date" density="compact"></v-text-field>
        <v-text-field v-model="current.daysInSprint" :rules="[required]" label="Days In Sprint" type="numeric" density="compact"></v-text-field>
        <v-text-field v-model="current.teamFlex" :rules="[required]" label="Team Flex %" type="numeric" density="compact"></v-text-field>
        <v-text-field v-model="current.pointsRolledOver" label="Points Rolled Over" type="numeric" density="compact"></v-text-field>
        <v-row>
          <v-col class="v-col-12 v-col-md-4">
            <v-btn prepend-icon="$add" color="indigo" @click="onAdd" v-if="current[id]===undefined" :disabled="!form.valid || form.sending" class="w-100">Add Sprint</v-btn>
    ...
    <v-container v-if="!form.show">
      ...
      <v-btn prepend-icon="$add" color="indigo" @click="onNew" class="w-100 mt-16">New Sprint</v-btn>
    </v-container>
    <v-dialog
